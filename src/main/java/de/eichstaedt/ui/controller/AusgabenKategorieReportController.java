...
public class AusgabenKategorieReportController {
	...
	public String getAusgabenKategorieReport(ModelMap model) {
		...
		model.addAttribute("reports", (List<AusgabenKategorieReport>)reportRepository.findAll());
		counter = 1;
		
		logger.info("Generate the report with counter {} and all reports {} ",counter,((List<AusgabenKategorieReport>)reportRepository.findAll()).size());
		
		Object [][] reportArray = new Object[((List<AusgabenKategorieReport>)reportRepository.findAll()).size()+1][2];
		reportArray[0][0] = "Ausgabe";
		reportArray[0][1] = "Kategorie";
		
		((List<AusgabenKategorieReport>)reportRepository.findAll()).stream().forEach((bericht)->{
